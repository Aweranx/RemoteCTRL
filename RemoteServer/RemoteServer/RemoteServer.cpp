...
int MakeDirectoryInfo(CPacket& packet) {
    std::string strPath;
    //std::list<FILEINFO> lstFileInfos;
    if (packet.GetFilePath(strPath) == false) {
...
int DownloadFile(CPacket& packet) {
	std::string strPath;
	packet.GetFilePath(strPath);
	long long data = 0;
	FILE* pFile = NULL;
...
int MouseEvent(CPacket& packet)
{
	MOUSEEV mouse;
	if (packet.GetMouseEvent(mouse)) {
		DWORD nFlags = 0;
...
int DeleteLocalFile(CPacket& packet)
{
	std::string strPath;
	packet.GetFilePath(strPath);

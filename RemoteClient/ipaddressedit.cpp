...
IPAddressEdit::IPAddressEdit(QWidget *parent) : QWidget(parent)
{
    ...
    //设置IP地址校验过滤
    QRegularExpression regExp("^(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9]?[0-9])$");
...
QString IPAddressEdit::getIP() const
{
    return this->ip;
}
...
void IPAddressEdit::setIP(const QString &ip)
{
    //先检测IP地址是否合法
    QRegularExpression regExp("((2[0-4]\\d|25[0-5]|[01]?\\d\\d?)\\.){3}(2[0-4]\\d|25[0-5]|[01]?\\d\\d?)");

    QRegularExpressionMatch match = regExp.match(ip);
    // match.hasMatch(): 是否找到了符合规则的子串
    // match.capturedLength() == ip.length(): 匹配的内容长度是否等于原字符串长度（即完全匹配）
    if (!match.hasMatch() || match.capturedLength() != ip.length()) {
        // 校验失败
        return;
    }
...
void IPAddressEdit::setBgColor(const QString &bgColor)
{
